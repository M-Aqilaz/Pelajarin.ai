    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1.3fr_0.7fr] gap-6">
        <section class="glass-panel rounded-2xl border border-white/5 p-6 space-y-4">
            ...
            <form action="{{ route('matches.messages.store', $match) }}" method="POST" class="space-y-4">
                ...
                <button class="rounded-xl bg-purple-600 px-6 py-3 text-white font-medium">Kirim</button>
            </form>
        </section>
        <aside class="glass-panel rounded-2xl border border-white/5 p-6 space-y-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-3">{{ $partner?->studyProfile?->bio }}</p>
            </div>
