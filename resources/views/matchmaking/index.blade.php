    ...
    <div class="grid grid-cols-1 xl:grid-cols-[0.95fr_1.05fr] gap-6">
        <section class="glass-panel rounded-2xl border border-white/5 p-6 space-y-6">
            ...
            <form method="POST" action="{{ route('matchmaking.profile.update') }}" class="space-y-4">
                ...
                <label class="flex items-center gap-3 text-sm text-gray-300"><input type="checkbox" name="is_matchmaking_enabled" value="1" @checked(old('is_matchmaking_enabled', $user->studyProfile?->is_matchmaking_enabled ?? true))> Aktifkan study matching</label>
                <button class="rounded-xl bg-white/10 px-5 py-3 text-white font-medium">Simpan Profil</button>
            </form>
        </section>
        ...
        <section class="glass-panel rounded-2xl border border-white/5 p-6 space-y-6">
            <div class="flex items-center justify-between">
                ...
                @if ($activeMatch)
                    <a href="{{ route('matches.show', $activeMatch) }}" class="rounded-xl bg-purple-600 px-4 py-2 text-sm text-white">Lanjutkan Match</a>
                @endif
            </div>
            ...
            @if ($queue)
                <div class="rounded-2xl border border-blue-500/30 bg-blue-500/10 p-5">
                    ...
                    <form method="POST" action="{{ route('matchmaking.cancel') }}" class="mt-4">@csrf<button class="rounded-xl bg-white/10 px-4 py-2 text-sm text-white">Batalkan Antrean</button></form>
                </div>
            @else
                <form method="POST" action="{{ route('matchmaking.search') }}" class="space-y-4">
                    ...
                    <button class="rounded-xl bg-purple-600 px-6 py-3 text-white font-medium">Mulai Matching</button>
                </form>
            @endif
        </section>
    </div>
