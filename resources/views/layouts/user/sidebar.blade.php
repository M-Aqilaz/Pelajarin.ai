<aside class="glass-panel-strong z-20 hidden h-full w-64 shrink-0 flex-col border-r border-white/10 md:flex">
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
        ...
        <div class="pt-4 pb-2"><p class="user-kicker px-3 text-[10px] font-semibold text-slate-400">Pembelajaran AI</p></div>
        ...
        <div class="pt-4 pb-2"><p class="user-kicker px-3 text-[10px] font-semibold text-slate-400">Bagian Fokus</p></div>
        ...
        <div class="pt-4 pb-2"><p class="user-kicker px-3 text-[10px] font-semibold text-slate-400">Belajar Sosial</p></div>
    ...
    <div class="border-t border-white/10 p-4">
        <div class="space-y-3">
            <div class="rounded-2xl border border-white/10 bg-white/[0.06] p-3">
                ...
                <p class="mt-2 text-[10px] uppercase text-violet-200">Paket {{ Auth::user()->plan }} | Cocok {{ Auth::user()->match_credits }}</p>
            </div>
            @if (Auth::user()->plan === 'free')
                <a href="{{ route('pricing') }}" class="block rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-3 transition-all hover:bg-cyan-400/15">
                    <p class="user-kicker text-[10px] text-cyan-100">Paket Gratis</p>
                    <p class="mt-2 text-sm font-semibold text-white">Naikkan ke Pro</p>
                    <p class="mt-1 text-xs text-cyan-100/80">Buka batas ruang yang lebih besar, kuota cocok lebih banyak, dan akses fitur premium.</p>
                </a>
            @endif
            ...
            @if (Auth::user()->role === 'admin')
                <a href="{{ route('admin.dashboard') }}" class="block rounded-xl px-3 py-2 text-xs text-slate-300 transition-all hover:bg-white/[0.06] hover:text-white">Dasbor Admin</a>
            @endif
